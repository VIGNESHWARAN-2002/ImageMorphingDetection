...
def copy_move_forgery():
    ...
    if path is None:
        # Show error message
        messagebox.showerror('Error', "Please select image to detect morphing")
        return
...
def metadata_analysis():
    ...
    if img_exif is None:
        # print('Sorry, image has no existing data.')
        # Retrieve the output image and display in resultPanel
...
root = Tk()
root.title("Image Morphing Detection")
...
resultLabel = Label(text="IMAGE MORPHING DETECTION", font=("Times", 60))
...
image_stegnography.grid(row=6, column=0, columnspan=2, pady=5)
